...
var expData = [{
    ...
    },
    {
        ...
        "start": "2019/04/01",
        "end": "2019/05/10",
        "content": "1개월간 온라인 강의 수강 후, '데이터에서 스토리 탐색하기'를 주제로 주어진 주식 데이터를 이용하여 10일간 데이터 분석 과제를 수행하였습니다.",
        "results": ["dsc2019"]
    },
    {
...
var resultData = {
    ...
    "cs50": {
        "skills": ["c"],
        "contents": {
            "활동내용": "Edwith의 [부스트코스] 모두를 위한 컴퓨터 과학(CS50 2019)을 기반으로 매주 강의 수강과 주어지는 미션들을 해결하고, 화상회의를 통해 스터디 통해 미션에 대하여 토론하는 방식으로 진행되었습니다.",
            "리더활동": "모든 팀원이 개발 관련하여 백지상태라는 문제점이 있었습니다. 이에 따라 리더로 활동하며 미션 접근 방법에 대해 조언을 하였고, 이해하지 못한 내용과 개념에 관해 설명하는 역할을 하였습니다.",
    ...
    "nts": {
        ...
        "contents": {
            ...
            "멘토링": "멘토를 통해 기술적인 궁금증을 해결할 수 있었고, Git lab을 이용한 개인 미션 코드 리뷰를 통해 부족한 점을 빠르게 채워나갈 수 있었습니다.",
            "결과": "인턴 과정을 통해 HTML, CSS 등 관련 기술뿐만이 아니라 웹 표준, 접근성, 크로스 브라우징과 같은 내용도 학습할 수 있었고, UI 개발 실무를 간접적으로 경험해 볼 수 있는 경험을 할 수 있었습니다.",
        },
        "links": {}
    },
    "dsc2019": {
        "skills": ["python"],
        "contents": {
            "주제": "주가 등락률로 확인하는 AI 취업 전망",
            "기간": "2019/05/1 - 2019/05/11",
            "팀구성": "개인",
            "역할": "데이터 분석",
            "내용": "경제 지표 데이터로 어떤 스토리를 만들 수 있을까? 라는 고민 끝에 '주가의 상승률은 투자자들의 해당 회사에 대한 성장 혹은 수익 증가에 따른 주가 상승에 대한 기대이고, 결국 이러한 기대와 대세를 따르기 위해 기업에서도 AI 활용을 고려할 것이다.'라는 가설을 세웠습니다. 이를 통해 빠르게 AI를 도입하고 있는 NAVER의 AI 이슈에 따른 주가 등락률 변화를 이용하여 데이터 분석을 진행했습니다.",
    ...
    "deep-studio": {
        ...
        "contents": {
            ...
            "팀구성": "영상합성(4), 음성합성(3)",
            "역할": "음성합성(데이터 전처리 도구 개발), 웹 구현",
            "내용": "딥러닝을 활용한 영상합성, 음성합성 기술을 이용하여 새로운 영상물 제작",
            "텍스트 수집": "음성 파일에서 Clova API를 이용하여 텍스트를 추출하는 도구를 Python으로 구현하였습니다.",
            "텍스트 전처리": "추출한 텍스트를 모델에 맞는 형식으로 처리하는 도구를 Python으로 구현하였습니다.",
            "음성 파일 전처리": "수집된 음성파일이 학습 과정에서 Out of Memory에러를 발생시켜 원인 분석 후 SoX를 활용한 Shellscript로 음성 파일을 Downsampling 하는 도구를 구현하였습니다.",
            "카탈로그 페이지 구현": "프로젝트 결과물들을 한 번에 보여줄 카탈로그 웹을 구현하였습니다.",
    ...
    "pommelier": {
        ...
        "contents": {
            ...
            "안주 추천 알고리즘 구현": "instacart의 식재료 데이터를 통해 와인과의 연관 규칙으로 같이 많이 구매한 상품들을 찾을 수 있었습니다. 이러한 결과로 만들 수 있는 요리를 와인 유형을 키로, 값을 음식들의 배열로 딕셔너리를 선언하여 임의로 불러오는 방식으로 구현했습니다.",
            "와인 추천결과 페이지 구현": "입력받은 사용자 인적 사항에 따라 분류된 와인 유형을 바탕으로 와인을 추천하였습니다. 추천된 와인의 특성을 Chart.js를 이용하여 시각적으로 확인할 수 있게 구현하고, 안주 추천 결과를 확인할 수 있게 구현하였습니다.",
    ...
    "like-lion": {
        "skills": ["ruby", "rails", "html", "css", "bootstrap"],
        "contents": {
            "내용": "3학년을 마치고 부족하다고 느꼈던 개발 경험을 쌓기위해 운영진으로 참여하였습니다.",
            "운영진 교육": 'Ruby의 기본적인 문법과 Rails 활용법, 적용되어 있는 MVC 패턴, REST 구조 등 웹 개발에 필요한 기본적인 지식에 대하여 학습하였습니다.',
            "운영진 활동": '교육 담당 운영진으로 활동하며 웹 개발과 관련된 전반적인 기초 지식들을 동아리원들에게 교육하였습니다.',
            "아이디어톤": '팀 리더로 활동하며 "SNS형 쇼핑 플랫폼 minor-market"을 기획하였습니다.',
            "헤커톤": '알까기 알고리즘을 만들어 대결하는 알까고에 참여하여 준우승 하였습니다.',
            "결과": '교육 담당 운영진으로 동아리원에게 학습 내용들을 교육하기위해 꼼꼼히 공부하며, 웹 개발에 대한 전체적인 흐름을 파악할 수 있었던 경험을 했습니다.',
        },
        "links": {}
    },
    "minor-market": {
        ...
        "contents": {
            ...
            "팀구성": "개인",
            "역할": "웹 구현",
            "내용": '멋쟁이 사자처럼의 아이디어톤을 통해 SNS 기반 쇼핑 플랫폼 minor-market을 기획하였습니다. 나아가 빅데이터 수집과 활용에 대한 포석으로 결제 시스템까지 결합하여 졸업 작품까지 진행하였습니다. 처음 SNS 기반 쇼핑 플랫폼을 기획하게 된 계기는 인디 밴드, 작은 공방의 생산품 등과 같은 마이너 콘텐츠들이 광고, 홍보에 대한 어려움으로 빛을 보지 못해 문을 닫게 되는 상황을 보고 이런 문제를 어떻게 해결하면 좋을까?라는 고민을 통해 만들게 되었습니다. 인스타그램, 블로그를 이용한 바이럴 마케팅으로 제품을 홍보하여 수익을 창출하는 인스타 마켓, 블로그 마켓에서 아이디어를 얻었고 이러한 마이너 콘텐츠들을 마치 플리마켓처럼 SNS 형태로 제공하면 충분히 경쟁력 있는 서비스를 만들 수 있겠다고 생각되어 시작했습니다.',
            "로그인, 회원가입": 'devise Gem을 이용하여 로그인, 회원가입 기능을 구현 하였습니다.',
            "유저 등급 구현": 'Rolify, CanCanCan gem을 이용하여 일반 사용자, 판매자, 관리자 별 권한을 구현하였습니다.',
            "프로필 페이지 구현": '유저와 상점별로 프로필 페이지를 구현하였습니다.',
            "팔로우, 좋아요 구현": 'CRUD를 응용하여 팔로우, 좋아요 기능을 구현하였습니다.',
            "댓글 구현": '등록된 상품과 글들에 댓글을 작성할 수 있게 구현하였습니다.',
            "타임라인 페이지 구현": '팔로우 된 유저의 개시글들을 확인할 수 있는 타임라인을 구현하였습니다.',
            "상품 등록 구현": '판매자가 상품을 등록할 수 있는 페이지를 구현하였습니다.',
            "상세 정보 페이지 구현": "상품의 상세 정보를 확인할 수 있는 페이지를 구현하였습니다.",
            "장바구니": '쿠키를 활용하여 장바구니 기능을 구현하였습니다.',
            "결제 기능 구현": '카카오페이 API를 이용하여 테스트 결제 기능을 구현하였습니다.',
